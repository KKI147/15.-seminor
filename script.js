...
$(document).ready(function () {
    ...
    const appW = HOLD_PANEL + BOARD_PAD * 2 + COLS * BLOCK;
    ...
    const boardOriginX = HOLD_PANEL + BOARD_PAD;
    ...
    const holdLabelStyle = new PIXI.TextStyle({
        fill: 0xc9d1d9,
        fontFamily: "Segoe UI, system-ui, sans-serif",
        fontSize: 13,
        fontWeight: "600",
    });
    const holdLabel = new PIXI.Text("HOLD", holdLabelStyle);
    holdLabel.x = 12;
    holdLabel.y = 8;
    holdLabel.zIndex = 7;
    ...
    function drawPanelAndBorder() {
        panelGfx.clear();
        panelGfx.beginFill(0x0d1117, 1);
        panelGfx.drawRoundedRect(0, 0, HOLD_PANEL - 8, appH, 8);
        panelGfx.endFill();

        borderGfx.clear();
        borderGfx.lineStyle(2, 0x30363d, 1);
        borderGfx.drawRoundedRect(
            boardOriginX - 2,
            boardOriginY - 2,
            COLS * BLOCK + 4,
            ROWS * BLOCK + 4,
    ...
    function drawHold() {
        holdGfx.clear();
        const boxCx = HOLD_PANEL / 2 - 4;
        const boxCy = appH / 2 + 6;
        const scale = 0.62;
        const b = BLOCK * scale;
        if (!hold) return;
        ...
        for (let y = 0; y < ph; y++) {
            for (let x = 0; x < pw; x++) {
                ...
                holdGfx.drawRoundedRect(px + 1, py + 1, b - 2, b - 2, 3);
                holdGfx.endFill();
